...
class VerticalConvolution(MaskedConvolution):
...
class HorizontalConvolution(MaskedConvolution):
    def __init__(self, in_channels, out_channels, kernel_size=3, mask_center=False, **kwargs):
...
class PixelCNN(GMM):
    ...
    def __init__(self, 
        ...
        super().__init__(n_gaussians, noise_mean, noise_std)
        
        out_channels = in_channels * n_gaussians * 3

        self.up_inconv = VerticalConvolution(in_channels, n_filters, kernel_size, mask_center=True)
        self.left_inconv = HorizontalConvolution(in_channels, n_filters, kernel_size, mask_center=True)
        self.dropout = nn.Dropout(dropout)
                
        self.gatedconvs = nn.ModuleList([GatedConv(n_filters, kernel_size=kernel_size)] * depth)
    ...
    def forward(self, x):
        ...
        for layer in self.gatedconvs[1:]:
            up = self.dropout(up)
