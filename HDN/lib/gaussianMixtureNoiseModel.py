...
class GaussianMixtureNoiseModel:
    ...
    def polynomialRegressor(self, weightParams, signals):
        ...
        for i in range(weightParams.shape[0]):
            value += weightParams[i] * (((signals - self.min_signal.to(torch.device('cuda'))) / (self.max_signal.to(torch.device('cuda')) - self.min_signal.to(torch.device('cuda')))) ** i);
        return value
    ...
    def loglikelihood(self, observations, signals):
        ...
        return torch.log(p+self.tol.to(torch.device('cuda')))
    ...
    def getGaussianParameters(self, signals):
        ...
        for num in range(kernels):
            ...
            alpha.append(torch.exp(self.polynomialRegressor(self.weight[2*kernels+num, :].to(torch.device('cuda')), signals) + self.tol.to(torch.device('cuda'))))
    ...
    def train(self, signal, observation, learning_rate=1e-1, batchSize=250000, n_epochs=2000, name= 'GMMNoiseModel.npz', lowerClip=0, upperClip=100):
        ...
        for t in range(n_epochs):
            ...
            p = self.loglikelihood(observations, signals)
            loss=torch.mean(-p)
